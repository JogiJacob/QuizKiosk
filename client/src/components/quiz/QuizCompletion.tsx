...
export function QuizCompletion() {
  ...
  const saveQuizSession = async (resultsData: any, participant: any) => {
    ...
    try {
      ...
      toast({
        title: "Warning",
        description:
          "Your quiz was completed but results could not be saved to the leaderboard.",
        variant: "destructive",
      });
    } finally {
      setSaving(false);
  ...
  const getScoreBadge = (percentage: number) => {
    ...
    if (percentage >= 70)
      return { text: "Good Job!", color: "bg-blue-100 text-blue-700" };
    if (percentage >= 50)
      return { text: "Not Bad!", color: "bg-yellow-100 text-yellow-700" };
    ...
    <div className="p-6 max-w-3xl mx-auto">
      <Card>
        <CardContent className="p-8 text-center">
          ...
          <div
            className={`inline-block px-4 py-2 rounded-full mb-6 ${badge.color}`}
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-muted/30 p-6 rounded-xl">
              <div
                className={`text-3xl font-bold mb-2 ${getScoreColor(results.percentage)}`}
